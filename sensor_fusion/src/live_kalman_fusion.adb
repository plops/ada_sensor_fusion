-- Live Kalman Fusion Pipeline
-- Real-time sensor fusion with streaming Kalman filter output
...
procedure Live_Kalman_Fusion is
   ...
   procedure Update_Kalman_Filter 
     (State : in out Kalman_State;
   ...
   begin
      ...
      DT := Float (Ada.Calendar.To_Duration (Clock - State.Last_Update));
      State.Last_Update := Clock;
      
      if DT > 0.0 and DT < 0.2 then
         -- Simple complementary filter (Kalman-like behavior)
         declare
            Accel_Mag : constant Float := 
              Accel.Accel_X**2 + Accel.Accel_Y**2 + Accel.Accel_Z**2;
         begin
            Norm_Factor := 1.0;
            if Accel_Mag > 0.01 then
               Norm_Factor := 1.0 / (Accel_Mag**0.5);
            end if;
   ...
   begin
      ...
      if Last > Buffer'First then
         ...
         begin
            ...
            Put_Line ("[" & Ada.Calendar.Image (Clock) & "] Received " & Natural'Image (Natural (Last)) & 
            ...
            begin
               if JSON_Body'Length > 0 then
                  ...
                  if Current_Accel.Valid or Current_Gyro.Valid then
                     Update_Kalman_Filter (Fusion_State, Current_Accel, Current_Gyro);
                     
                     -- Stream fusion output
                     Put_Line ("=== KALMAN FILTER OUTPUT ===");
                     Put_Line ("Timestamp: " & Ada.Calendar.Image (Clock));
                     Put_Line ("Quaternion: [" & 
                               Float'Image (Fusion_State.Q_X) & ", " &
                               Float'Image (Fusion_State.Q_Y) & ", " &
                               Float'Image (Fusion_State.Q_Z) & ", " &
                               Float'Image (Fusion_State.Q_W) & "]");
                     Put_Line ("Gyro Bias: [" &
                               Float'Image (Fusion_State.Bias_X) & ", " &
                               Float'Image (Fusion_State.Bias_Y) & ", " &
                               Float'Image (Fusion_State.Bias_Z) & "]");
                     ...
                     declare
                        ...
                        Yaw   : Float;
                     begin
                        Roll := 2.0 * (Fusion_State.Q_W * Fusion_State.Q_X + 
                                       Fusion_State.Q_Y * Fusion_State.Q_Z);
                        Pitch := 2.0 * (Fusion_State.Q_W * Fusion_State.Q_Y - 
                                        Fusion_State.Q_X * Fusion_State.Q_Z);
                        Yaw := 2.0 * (Fusion_State.Q_W * Fusion_State.Q_Z + 
                                      Fusion_State.Q_X * Fusion_State.Q_Y);
...
begin
   Put_Line ("=== Live Kalman Fusion Pipeline ===");
   ...
   Put_Line ("✓ Kalman filter initialized");
