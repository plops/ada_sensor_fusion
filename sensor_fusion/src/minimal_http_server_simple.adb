...
procedure Minimal_HTTP_Server_Simple is
   ...
   procedure Handle_Sensor_Data (Body_Str : String) is
      use GNATCOLL.JSON;
      use Ada.Calendar;
   ...
   begin
      Put_Line ("Processing sensor data: " & Natural'Image (Body_Str'Length) & " bytes");
      
      -- Show first 100 characters of data for debugging
      if Body_Str'Length > 0 then
         declare
            Show_Length : constant Natural := Natural'Min (Body_Str'Length, 100);
         begin
            Put_Line ("Data preview: " & Body_Str (Body_Str'First .. Body_Str'First + Show_Length - 1));
         end;
      end if;
            ...
            loop
               ...
               begin
                  ...
                  if To_String (Method) = "POST" and then To_String (URI) = "/data" then
                     Handle_Sensor_Data (To_String (Request_Body));
...
exception
   when E : others =>
      ...
      Finalize;
end Minimal_HTTP_Server_Simple;
