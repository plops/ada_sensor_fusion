-- Simple Working HTTP Server for Sensor Data
...
procedure Simple_HTTP_Server_Working is
...
begin
   Put_Line ("=== Simple Working Ada HTTP Server ===");
   ...
   loop
      begin
         ...
         begin
            -- Send response character by character
            for I in Response'Range loop
               declare
                  Item : constant Ada.Streams.Stream_Element := 
...
exception
   when E : others =>
      ...
      if Is_Open (Server_Socket) then
         Close_Socket (Server_Socket);
      end if;
end Simple_HTTP_Server_Working;
